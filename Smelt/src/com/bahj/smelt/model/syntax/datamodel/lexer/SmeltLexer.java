...
public class SmeltLexer {
    ...
    /**
     * Used to keep track of queued tokens. A single lexer operation may generate multiple tokens, so the spares are
     * kept here until they are needed.
     */
    ...
    private void pushback() throws LexerException {
        if (!this.lastValid) {
            throw new IllegalStateException("Cannot push back when no valid character exists!");
        } else {
            this.lastValid = false;
            if (this.lastChar != -1) {
                try {
                    ...
                    throw new IllegalStateException("I/O exception on pushback of character", ioe);
                }
    ...
    /**
     * Reads characters until the specified predicate fails. The failed character is pushed back.
     * 
     * @param pred
     *            The predicate.
    ...
    /**
     * Generates the next token found in the reader or <code>null</code> if no such token exists.
     * 
    ...
    /**
     * Determines if another token exists to read.
     * @return <code>true</code> if another token exists; <code>false</code> if it does not.
     */
    ...
    /**
     * Creates a view of this lexer as an iterator.  Unlike the {@link Iterable} interface, this does <i>not</i> create a new
     * iteration each time it is called.
     * @return A view of this object as an {@link Iterator}.
     */
    public Iterator<Token> asIterator() {
        return new Iterator<Token>(){

            @Override
            public boolean hasNext() {
                return SmeltLexer.this.hasNextToken();
            }

            @Override
            public Token next() {
                return SmeltLexer.this.nextToken();
            }
        };
    }
    ...
    /**
     * Tries to find tokens in the reader. By the end of this operation, some tokens may have been added to the token
     * queue.
     * 
     * @return <code>true</code> if there may be more tokens in the future; <code>false</code> if there definitely won't
     *         be.
     * @throws IOException
     *             If an I/O error occurs.
     * @throws LexerException
        ...
        if (c == -1) {
            if (!this.emittedLastToken) {
                ...
                while (!this.indentationLevels.isEmpty()) {
                    ...
                    this.tokenQueue.offer(new DedentToken(new Span(start, this.location)));
                }
                // Emit the last token.
                this.tokenQueue.offer(new EndOfFileToken(new Span(start, this.location)));
            ...
            // indentation stack.
            start = this.location;
            String spaceString = readWhile((int ch) -> Character.isWhitespace(ch) && ch != '\n');
            ...
            // token.
            this.tokenQueue.offer(new EndOfLineToken(new Span(start, this.location)));
            ...
            int currentIndentation = this.indentationLevels.isEmpty() ? 0 : this.indentationLevels.peek();
            if (spaces > currentIndentation) {
                // Then this is deeper. Emit an indentation and record the
                // level.
                this.indentationLevels.push(spaces);
                this.tokenQueue.offer(new IndentToken(new Span(start, this.location)));
                ...
                while (!this.indentationLevels.isEmpty() && this.indentationLevels.peek() > spaces) {
                    this.tokenQueue.offer(new DedentToken(new Span(start, this.location)));
                    this.indentationLevels.pop();
                }
                currentIndentation = this.indentationLevels.isEmpty() ? 0 : this.indentationLevels.peek();
            ...
            this.tokenQueue.offer(new ColonToken(new Span(start, this.location)));
            return true;
        } else if (c == ',') {
            this.tokenQueue.offer(new CommaToken(new Span(start, this.location)));
            return true;
        } else if (c == '=') {
            this.tokenQueue.offer(new EqualToken(new Span(start, this.location)));
            return true;
        } else if (Character.isAlphabetic(c) || Character.isDigit(c)) {
            // Then we have found a text term. Read until we can't find more
            // text.
            pushback(); // put c back in the reader
            String text = readWhile((int ch) -> (Character.isAlphabetic(ch) || Character.isDigit(ch)) && ch != '\n');
            this.tokenQueue.offer(new IdentifierToken(new Span(start, this.location), text));
            return true;
        } else {
